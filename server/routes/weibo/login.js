...
function pageLogin(req, res, next) {
    ...
    proxy.apiProxy(apiUrl, params, async function(err, body) {
        ...
        }else if(req.query._act === '1'){
	        // 三方域名活动授权时，使用它对应的开放平台id和授权跳转页面
	        try {
		        let params = {
			        domain: req.get('Host'),
		        };
		        let result = await proxy.apiProxyPromise(conf.baseApi.getWxAppIdByDomain, params, conf.baseApi.secret);
		        if (result && result.state && result.state.code === 0) {
			        wxOpenAppId = result.data.openAppId;
		        }
	        } catch (err) {
		        console.error(err);
	        }
...
function isWeiboRoutePage (url) {
    return /\/live\/(\w)*\.htm/.test(url) ||
        /\/wechat\/page\/live/.test(url) ||
        /\/live\/channel\/channelPage\/(\w)*\.htm/.test(url) ||
        /\/wechat\/page\/topic\-intro/.test(url) ||
