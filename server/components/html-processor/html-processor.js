...
var handle = function(req, res, next, opts) {
    ...
    if(userTag){
	    options.renderData = options.renderData || {};
	    options.renderData.USER_TAG = userTag;
    }
    ...
    // 如果该页面被标记为静态化页面，就把该页面更新进缓存
    if (req.isStaticHtml) {
        staticHtml.setHtmlCache(req.staticHtmlPrefix, req.staticHtmlId, html, req.staticHtmlExpires);
    }
