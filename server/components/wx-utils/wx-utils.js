var _ = require('underscore'),
    ...
    conf = require('../../conf'),
    tokenRedis = require('../token-redis'),
...
function getConfig (appId,callback){
    var tokenRedisCluster = appId ? tokenRedis.getActCluster() : tokenRedis.getCluster();
    if(!tokenRedisCluster) {
    ...
    tokenRedisCluster.get(key, function(err, config){
        if(err){
            config = {};
        } else {
            try {
 ...
 * @return   {[type]}                                    [description]
 */
module.exports.getAuthLoginUrl = function(pageUrl, appId, scope, state, callback) {
 ...
 * @return {[type]}         [description]
 */
module.exports.goWxAuth = async function(req, res, pageUrl) {
    ...
    if (req._isGoKnowledgeToQlCodeAuth) {
        ...
        try {
            ...
            } else if (req.query.fromLiveId) {
                params.liveId = req.query.fromLiveId;
            } else if (req.query.campId) {
                params.campId = req.query.campId;
            } else if (req.params.campId) {
                params.campId = req.params.campId;
            } else if (req.query.id && req.path.indexOf('homework') > -1) {
                // 这里的id没有区分，所以额外加上path的简单判断判断为作业
                params.homeworkId = req.query.id;
            }

            let result = await proxy.apiProxyPromise(conf.baseApi.isLiveAdmin, params, conf.baseApi.secret, req);
    ...
    if (!envi.isWeixin(req)) {
        ...
        let wxLoginPageUrl = '/page/login?redirect_url=' + encodeURIComponent(pageUrl);
...
// /**
//  * 判断是否通过知识店铺公众号授权
//  * 直播间主页、系列课主页、话题详情页（包含音视频）、音频图文
//  * @param  {[type]}  req [description]
//  * @return {Boolean}     [description]
//  */
// function isGoKnowledgeCodeAuth(url) {
//
//         return /\/topic\/details\?/.test(url) ||
//             /\/wechat\/page\/live\/(\w)*/.test(url) ||
//             /\/live\/channel\/channelPage\/(\w)*\.htm/.test(url) ||
//             /\/topic\/details-audio-graphic\?/.test(url) ||
//             /\/topic\/details-video\?/.test(url) ||
//             /\/wechat\/page\/get-coupon-vip\/\d+\?/.test(url) ||
//             /\/wechat\/page\/get-coupon-channel\/\d+\?/.test(url) ||
//             /\/wechat\/page\/get-coupon-topic\/\d+\?/.test(url);
// }
