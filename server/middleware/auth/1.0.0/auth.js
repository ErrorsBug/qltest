...
var validate = function(opts) {
    ...
    return function(req, res, next) {
		...
		var isFail = function() {
			options.checkUser(username, password, req, res, function(err, user) {
				...
				if (user && user.userId) {
					...
					var isHttps = conf.mode === 'development' || conf.mode === 'prod';
					var pageUrl = (isHttps ? 'https' : req.protocol) + '://' + req.get('Host') + req.originalUrl;
					var newPageUrl = urlUtils.fillParams({

					}, pageUrl, ['code', 'loginType', 'state', 'authDataKey', '_klca', '_kltoqlca', 'client', '_klUserId']);
					if (pageUrl === newPageUrl) {
                        // console.log('newPageUrl==========', newPageUrl)
						_routeProcess(req, res, next, options.successRedirect);
					} else {
                        // console.log('newPageUrl==========newPageUrl', newPageUrl)
        ...
        // 已验证通过
		var isLoginRseult = options.isLogined(req, res)
        // console.log(isLoginRseult, 'isLoginRseult==================')
		if(isLoginRseult instanceof Promise) {
			isLoginRseult.then(flag => {
				if (flag) {
					isOk()
				} else {
					isFail()
...
function updateAuthUserSession(req, res, user, options) {
    ...
    // console.log(req.rSession, 'req.rSession=================')
