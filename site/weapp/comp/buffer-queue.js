...
class BufferQueue {
    constructor() {
        ...
        this.__event.on('GLOBAL_TIMEUP', this.disposeAll.bind(this));
        this.__event.on('FLUSH', this.flush.bind(this));
        this.__event.on('DEQUEUE', this.dequeue.bind(this));
    }
    ...
    enqueue(type, item) {
        ...
        // console.log('自身缓存限制: ', this.__BufferMap.get(type).limit);
        // console.log('已缓存长度: ', buffers.length);

        if (buffers.length > this.__BufferMap.get(type).limit) {
            // console.log(buffers.length,this.__BufferMap.get(type).limit)
    ...
    setAutoDequeueTimeLimit(type) {
        if (this.__BufferMap.get(type).buffers.length <= 3) {
            this.__BufferMap.get(type).autoDequeueTimeLimit = 1000;
        } else if (this.__BufferMap.get(type).buffers.length <= 6) {
            this.__BufferMap.get(type).autoDequeueTimeLimit = 500;
        } else if (this.__BufferMap.get(type).buffers.length <= 12) {
            this.__BufferMap.get(type).autoDequeueTimeLimit = 333;
        } else if (this.__BufferMap.get(type).buffers.length <= 20) {
     ...
     * @memberof BufferQueue
     */
    setTypeTimer(type, time) {
        if (!type || typeof(time) !== 'number' || time < 100) {
            throw '参数输入错误，type 必须存在, time 必须为数字且大于等于100';
        }
        ...
        this.__BufferMap.get(type).timer = setInterval(() => {
    ...
    clearTypeTimer(type) {
        ...
        clearInterval(this.__BufferMap.get(type).timer);
        this.__BufferMap.get(type).timer = null;
    }
    ...
    flush(type) {
        ...
        // console.log('触发 flush 函数');
        // console.log('flush type：', type);

        if (bufferObj.buffers.length !== 0) {
            // console.log('调用 handler 函数');
    ...
    dequeue(type) {
       if (!type) {
        ...
        if (bufferObj.buffers.length !== 0) {
            // console.log('调用 handler 函数');
            // console.log(bufferObj.buffers.shift())
