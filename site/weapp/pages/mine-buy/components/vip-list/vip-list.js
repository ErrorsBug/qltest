...
Component({
    ...
    methods: {
        ...
        updateList(data) {
            ...
            data = data.map(item => ({
                ...item,
                pic: imgUrlFormat(item.pic, '?x-oss-process=image/resize,h_110,w_110,m_fill'),
                expiryTimeStr: formatDate(item.expiryTime, 'yyyy-MM-dd'),
                overtime: item.expiryTime < this.data.sysTime,
            }))
