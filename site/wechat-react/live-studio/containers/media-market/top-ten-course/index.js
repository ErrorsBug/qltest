...
@autobind
class TopTenCourse extends Component {
    ...
    uiCourseList = () => {
        return this.state.courseList.map((item, index) => {
            return (
                <div className="course-item-wrap" key={item.businessId}>
                    {
                        index < 3 ?
                        <div className="honor">
                            {index + 1}
    ...
    getCourseList = async (refresh) => {
        const isRecommend = 'Y';
        const result = await api({
            url: '/api/wechat/studio/hotCourseList',
            method: 'POST',
            body: {
                isRecommend,
                page: {
                    page: refresh ? 1 : this.state.pageNum + 1,
        ...
        if (result && result.state && result.state.code === 0 ) {
            const { hotCourseList } = result.data;
            const noMore = hotCourseList.length < this.data.pageSize ? true : false;
            let { courseList } = this.state;
            refresh ? courseList = hotCourseList : courseList = courseList.concat(hotCourseList)
            this.setState({
                courseList,
    ...
    render() {
        ...
        return (
            <Page title="知识通商城" className="top-ten-course">
...
export default connect(mapStateToProps, mapActionToProps)(TopTenCourse);
