...
@autobind
class FavourableCourse extends Component {
    ...
    getCourseList = async (refresh) => {
        const isRecommend = 'Y';
        const result = await api({
            url: '/api/wechat/studio/discountCourseList',
            method: 'POST',
            body: {
                isRecommend,
                page: {
        ...
        if (result && result.state && result.state.code === 0 ) {
            ...
            const noMore = list.length < this.data.pageSize ? true : false;
            let { courseList } = this.state;
            refresh ? courseList = list : courseList = courseList.concat(list);
            this.setState({
                courseList,
    ...
    render() {
        ...
        return (
            <Page title="知识通商城" className="favourable-course">
                <ScrollToLoad
...
export default connect(mapStateToProps, mapActionToProps)(FavourableCourse);
