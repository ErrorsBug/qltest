...
import { channelGroup } from '../../../../reducers/channel-group';
...
@autobind
class NightAnswerAudioPlayer extends PureComponent {
				...
				<div className="play-icon-con">
					{
						...
						<span className={`btn-play icon_audio_pause on-log ${this.state.showBtnLoading ? ' loading' : ''}`} onClick={this.pauseAudio.bind(this)} data-log-id={this.props.topicId} data-log-region="pause">
						<svg version="1.1"
						...
						:
						<span className={`btn-play icon_audio_play on-log`} onClick={this.playAudio.bind(this)} data-log-id={this.props.topicId} data-log-region="play"></span>
					}
				</div>
				<div className="audio-info">
					<div className="audio-name">
						<span>夜答第{this.props.num}期</span>
						<div className="history" onClick={this.props.showHistory}>
							<span className="icon"></span>往期
						</div>
					</div>
					<div className="audio-time">
						<span className="start">{this.getSecond()}</span>
						{/*底部播放条*/}
						{
							!!this.props.currentAudioUrl &&
							<div className="audio-bar-container" onClick={this.audioBarClickHandle}>
								<div className="audio-bar" ref="audioBar">
						...
						<span className="end">{this.convertSecond(this.state.audioDuration)}</span>
					</div>
				</div>
			</div>
        )
...
export default NightAnswerAudioPlayer