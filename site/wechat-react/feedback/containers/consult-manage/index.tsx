...
> {
    ...
    ajaxSetSelection = async () => {
        ...
        if (result.state.code == 0) {
            ...
            (window as any).toast(result.state.msg);
        }
    };

    onClickReply = (replyPeople, replyType, initialValue, id, replyId) => {
        this.setState({
            inputBox: {
                replyPeople,
                replyType,
    ...
    modifyReply = async (replyId, value) => {
        let result = await apiService.post({
    ...
    render() {
        return (
            <Page title={`留言管理`} className="consult-manage-container">
                <ScrollView
                ...
                >
                            ...
                            <Card
                                ...
                                onClick={() => {
                                    if (this.businessType == "channel") {
                                        ...
                                        location.href = `/wechat/page/topic-intro?topicId=${
                                            this.businessId
                                        }`;
                                    }
                                }}
                    ...
                    <MiddleDialog
                        show={!!this.state.setSelectionDialog}
                        bghide={true}
                ...
                {this.state.inputBox ? (
                    <InputBox
                        initialValue={this.state.inputBox.initialValue}
                        onBlur={() => {
                            this.setState({
                                inputBox: false
                            });
                        }}
                        placeholder={`回复：${this.state.inputBox.replyPeople}`}
                        onSend={this.ajaxReply}
                    />
                ) : null}
            </Page>
        );
