...
import { postCheckIn, writePaper } from '../../actions/camp'
...
export default class EditIdea extends Component {
    ...
    // 初始化数据
    async initData() {
        const { dataList } = await getTopicList({ source: 'ufw', ...this.page })
        ...
        this.page.page += 1;
        this.setState({
            topicLists: [...this.state.topicLists, ...dataList ]
        })
    }
    ...
    async saveIdea() {
        ...
        if(!!editType) {
            parmas.campId = this.props.campId;
            parmas.periodId = this.props.periodId;
            parmas.checkInTime = this.props.checkTime || new Date().getTime()
        }
        if(Object.is(editType, 'check')) {
            parmas.checkInType = this.props.checkTime ? 'APPEND' : 'NORMAL'
            const { result } =  await postCheckIn(parmas) || {}
            if(Object.is(result, 'Y')) {
                localStorage.removeItem('ideaInfo')
                localStorage.removeItem('ideaTxt')
                this.props.generatePoster && this.props.generatePoster(editType)
            } else {
                window.toast('打卡失败')
            }
        } else {
            ...
            try {
                const res = Object.is(editType, 'paper') ? await writePaper(parmas) : await addIdea(parmas);
                if((!Object.is(editType, 'paper') && !!res) || (Object.is(editType, 'paper') && Object.is(res?.result, 'Y'))) {
                    localStorage.removeItem('ideaInfo')
                    localStorage.removeItem('ideaTxt')
                    const type = Object.is(editType, 'paper') ? 'editPaper' : 'saveIdea'
                    PubSub.publish(type)
                    window.toast('发布成功')
                    this.props.handleShowEdit && this.props.handleShowEdit(); 
                } 
            } catch (error) {
                window.toast('打卡失败')
            }
    ...
    render() {
        const { value, len, imgMaxCount, count, isShowControll, imgs, topicName, isTopic, isMore, topicLists } = this.state;
        const { editType } = this.props;
        ...
        return (
            <Fragment>
                <PortalCom className="un-edit-idea-box">
                    <div className="un-edit-idea-head">
                        { Object.is(editType, 'check') && <h4>记录今日的心得感悟</h4> }
                    ...
                    <div className={ pubCls } 
                        ...
                        data-log-pos={ '0' } 
                        onClick={ this.saveIdea }>{ Object.is(editType, 'check') ? '生成打卡海报' : '发布' }</div>
                </PortalCom>
                { isTopic && !editType && (
                            ...
                            <div className="un-topic-items">
                                <ScrollToLoad
                                    ...
                                    { topicLists.map((item, index) => (
                                        <TopicItem key={ index } {...item} handleSelectTopic={ () => this.handleSelectTopic(item) }/> 
                                    )) }
