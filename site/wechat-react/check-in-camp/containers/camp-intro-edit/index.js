...
import { uploadImage, initStsAuth } from '../../actions/common';
...
class ChannelIntroEdit extends Component {
    ...
    componentDidMount() {
        if (this.props.params.campId) {
            this.loadChannelIntro(this.props.location.query.type,this.props.params.campId);
        }
        this.initStsInfo();
    }
    ...
    async saveCahnnelIntro(tag){
        if(tag=="confirm"){
            ...
            let result = await this.props.saveChannelIntro(this.props.location.query.type, this.props.params.campId,this.state.channelIntroList);
    ...
    async addIntroImageItem(event) {
        ...
        const stsAuth = this.props.stsAuth;
        uploadImage(file, stsAuth, 5, 'channelLogo').then((filePath) => {
            this.setState({
                filePath: filePath,
            });
            if (filePath) {
