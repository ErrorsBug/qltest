...
class SpecialsCountDown extends Component {
    ...
    async componentDidMount() {
        ...
        this.props.channelId && this.getDiscountExtend();
    }
    ...
    async getSysTime() {
        if(this.props.sysTimestamp){
            return this.props.sysTimestamp;
        }
    ...
    getDiscountExtend() {
        ...
        apiService.post({
            url: '/h5/channel/getDiscountExtend',
            body: {
                channelId:this.props.channelId
            }
        }).then(res => {
            let discountExtendPo = getVal(res, 'data.discountExtendPo', {});
            let leftTime = 0,leftStartTime =0;
            if(!!discountExtendPo.endTime){
                leftTime = getVal(res, 'data.discountExtendPo.endTime', 0);
                leftStartTime = getVal(res, 'data.discountExtendPo.startTime', 0);
            } else {
...
function mapStateToProps(state) {
    let channel = getVal(state, 'channel.chargeConfigs').length ? state.channel : state.channelIntro;
    let chargeConfigs = channel.chargeConfigs.length ? channel.chargeConfigs : getVal(state, 'topicIntro.channelInfo.chargeConfigs',[]);
    return {
