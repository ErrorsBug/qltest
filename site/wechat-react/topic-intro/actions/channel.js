...
export const INIT_DISCOUNT_EXTENDPO= 'INIT_DISCOUNT_EXTENDPO';
 ...
 * @return {[type]}             [description]
 */
export function fetchCourseList(channelId, liveId, page, size, showLoading, showDisplay = 'N') {
    return async (dispatch, getStore) => {
        const result = await api({
            dispatch,
            getStore,
...
///api/wechat/channel/getCouponBoxCourseInfo
...
/* 更新特价信息 */
export function updateDiscountExtendPo(discountExtendPo){
    return async (dispatch, getStore) => {
        dispatch({
            type: INIT_DISCOUNT_EXTENDPO,
            discountExtendPo
        });
