...
class BtnShareCard extends Component {
    ...
    initShareUrl() {
        let lshareKey = getVal(this.context.router, 'location.query.shareKey', '');
    ...
    render() {
        ...
        let hasLShareKey = lShareKey && lShareKey.shareKey && lShareKey.status == 'Y';
        let hasTopicShareKey = shareKey && shareKey.shareKey && shareKey.status == 'Y';
        // 有 coral.lsharekey 表示该课程有加入珊瑚计划； 
        // 有 coral.percent 表示我是珊瑚计划课代表
        let isCoral = coral && coral.lsharekey && coral.percent;
	    ...
	    if (topicInfo.isRelay == 'Y' || topicInfo.campId) {
	    ...
	    } else if (topicInfo.type == 'charge') {

            if(liveRole != 'creater' && (hasTopicShareKey || hasLShareKey ) && isCoral){
                // 如果是课代表并且有珊瑚计划身份
                return (
                    ...
                    >
                        <div className="main">
                            ...
                            <span className="content">
                                {/* 分享赚{formatMoney(topicInfo.money * this.props.distributionShareEarningPercent / 100)}元 */}
                                生成邀请卡
                            </span>
                        </div>
                    </div>
            ...
            }else if(topicInfo.isAutoshareOpen === 'Y' && !lShareKey.shareKey && !shareKey.shareKey){
                // 开启了自动分销 
	            ...
	            return (
                    ...
                    >
                        <div className="main">
                            <i className="icon-share"></i>
                            <span className="content">
                                {/* 分享赚{formatMoney(topicInfo.money * this.props.autoSharePercent / 100)}元 */}
                                生成邀请卡
                            </span>
                        </div>
                    </div>
