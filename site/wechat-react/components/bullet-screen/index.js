...
export default class BulletScreen extends React.Component {
	...
	componentWillMount() {
		// 读取开关状态
		let isSwitchOn = !detect.os.android;
		try {
			if (localStorage.getItem('BULLET_SCREEN_SWITCH_OFF') === '1') {
				isSwitchOn = false;
			} else if (localStorage.getItem('BULLET_SCREEN_SWITCH_OFF') === '0') {
				isSwitchOn = true;
			}
		} catch (e) {}
		this.setState({
	...
	componentWillReceiveProps(nextProps) {
        ...
        if (this.props.currentTime !== nextProps.currentTime && nextProps.playerStatus !== 'ended') {
            this.handleUpdateCurrentTime(Math.floor(nextProps.currentTime));
        }
	}

	componentDidUpdate() {
		const isShowSendBar = this.getIsShowSendBar();
		if (isShowSendBar !== this._preIsShowSendBar) {
			this.props.onChangeIsShowSendBar && this.props.onChangeIsShowSendBar(isShowSendBar);
		}
		this._preIsShowSendBar = isShowSendBar;
	}

	render() {
		let { bulletMap, rowsStatus, isPlay } = this.state;
		let now = Date.now();
		let topicId = this.props.topicInfo.id;
	...
	getIsShowSendBar = () => {
		return !!(this.state.isSwitchOn && (this.state.isShowSendBar || this.state.bulletInputText) && this.props.mainContainer);
	}

	showSendBar = () => {
		if (this.state.isShowSendBar) {
			this.setState({
				isShowSendBar: false,
				bulletInputText: '',
			})
		} else {
			this.setState({isShowSendBar: true}, () => {
				this.state.isShowSendBar && setTimeout(() => {
	...
	bulletInputBlurHandle(){
		this.hideSendBarTimer = setTimeout(() => {
			!this.state.bulletInputText && this.setState({
				isShowSendBar: false
			});
		}, 300)
	}
	...
	getBullets = (startTime = 0, isForward) => {
		...
		request({
			...
			body: {
				...
				startTime,
				endTime,
				count: Math.ceil(endTime - startTime) * 2,
			}
		}).then(res => {
			if (res.state.code) throw Error(res.state.msg);

			// 长度小于3的过滤掉
			let list = (res.data.list || []).filter(bullet => {
				return bullet.content && bullet.content.length > 2;
			});
			this.addBullet(list);
			// console.log(`get bullets: ${startTime}s-${endTime}s`, list);
    ...
    onClickSwitchOn = () => {
		...
		try { localStorage.setItem('BULLET_SCREEN_SWITCH_OFF', '0') } catch(e) {}
    }
...
let visibleLogs = [];
let visibleLogsTimer;
// 延时5秒发送
function sendVisibleLog(log) {
	if (typeof _qla === 'undefined') return;
	visibleLogs.push(log);

	if (visibleLogsTimer) return;

	visibleLogsTimer = setTimeout(() => {
		_qla('visible', {
			logs: JSON.stringify(visibleLogs),
		});
		visibleLogs = [];
		clearTimeout(visibleLogsTimer);
		visibleLogsTimer = null;
	}, 5000)
}
...
function throttleDebounce(fun, interval = 200) {
    let timer,
        trailing = false, // 标记计时器结束后是否需要再次执行
        context = null,
        args = null;
