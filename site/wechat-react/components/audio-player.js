...
import { getUrlParams, fillParams } from './url-utils';
...
export class AudioPlayer {
    constructor(id) {
        ...
        this.__playWav = false;
        this.__currentPlaybackRate = 1;
    }
	...
	// 获取 playbackRate
	get playbackRate() {
		return this.__currentAudio.playbackRate || 1;
	}
	...
	set playbackRate(rate){
        this.__currentPlaybackRate = rate || 1;
		this.__currentAudio.playbackRate = rate || 1;
	}
    ...
    resume() {
        ...
        let playPromise = this.__currentAudio.play();
        if (playPromise && playPromise instanceof Promise){
            playPromise.catch(() => { console.error('chrome stop autoPlay')})
        }
    ...
    __setCurrentAudio() {
	    ...
	    this.__currentAudio.playbackRate = this.__currentPlaybackRate;
    }
    ...
    __audioErrorHandle(e,msg,errorCode){
        // if (/docs.qianliaowang.com/.test(this.__playSrc)) return ;
        let audioTempSrc = "";
        let currentTime;
        if (e.target === this.__currentAudio) {
        ...
        } else if(!/(jump=200)/.test(audioTempSrc) && e.target === this.__currentAudio){
            const jump = parseInt(getUrlParams('jump', audioTempSrc)) || 0;
            audioTempSrc = fillParams({
                jump: jump + 1
            }, audioTempSrc);
        ...
        if (e.target === this.__currentAudio) {
            this.__retryPlay(audioTempSrc, currentTime);
        } else {
    ...
    __audioError(e) {
        ...
        if (this.__playSrc != '') {
            console.log('发生错误音频url：' + e.target.src);
    ...
    __setPlaySrc(src) {
        ...
        if (this.__playWav && /(\.wav)/.test(src)) {
            this.__playSrc = src;
        }else if (!/(\.mp3)/.test(src) && !/(\.\w+$)/.test(src)) {
            // 首先尝试使用 m4a 格式播放
    ...
    __retryPlay(src, currentTime) {
        this.__playSrc = src || '';
        this.__currentAudio.src = src || '';
        this.__currentAudio.currentTime = currentTime || 0;
        this.__setStatu(PLAYING);
    ...
    __intervalStart() {
        ...
        this.__cheakCurrentTime = setInterval(() => {
            ...
            if ( this.__statu === END || this.__statu === PAUSE) {
                this.__intervalStop();
                return;
            }
    ...
    error (f) {
        this.__errorCallback = f
    }
}

export class AudioSlicePlayer {
    constructor(percentCallback, endCallback, errorCallback, pauseCallback) {
        ...
        this.src = [];
        this.currentSliceIndex = 0; //当前播放分片的索引

        this.timeLogger = {
           
            audioDuration: 0, //当前播放音频的累计时间
            currentSliceTime: 0, //当前分片音频的播放时间
        }

        this.currentTime = this.timeLogger.audioDuration + this.timeLogger.currentSliceTime; //当前音频的播放时间
        ...
        this.audioPlayer.on('ended', (e) => {
            this.audioPlayer.currentTime = 0;
            if (this.currentSliceIndex < this.src.length - 1) {
                this.playNextSlice()
            } else {
                this.stop();
                this.timeLogger = {
    ...
    clearInterval () {
        this.audioPlayerTimer&&window.clearInterval(this.audioPlayerTimer);
    }

    setInterval () {
        this.audioPlayerTimer = setInterval(() => {
            if (this.audioPlayer.currentTime > 600) {
                console.log('超过10分钟！');
                this.stop()
            }
            this.timeLogger.currentSliceTime += 1;
            this.currentTime = this.timeLogger.currentSliceTime + this.timeLogger.audioDuration;
            this.percentCallback && this.percentCallback(this.currentPercent())
        },1000);
    }
    ...
    playNextSlice () {
        this.clearInterval();
        const idx = this.currentSliceIndex;
        this.currentSliceIndex = idx + 1;
        const audioDuration = this.timeLogger.audioDuration + this.timeLogger.currentSliceTime;
        this.timeLogger = {
            audioDuration: audioDuration,
            currentSliceTime: 0
        }
        this.currentTime = this.timeLogger.audioDuration + this.timeLogger.currentSliceTime;
        this.audioPlayer.play(this.src[this.currentSliceIndex])
    }

    addPercentListener (callback) {

        this.percentCallback = callback;
    }

    currentPercent () {
        return Number(this.currentTime / this.totalSecond * 100).toFixed(0)
    }

    setSrc (src, totalSecond) {
        this.src = src;
        this.totalSecond = totalSecond;
    }

    play (src, totalSecond) {
        this.setSrc(src, totalSecond);
        this.resetCurrentTime();
        this.audioPlayer.pause();
        this.audioPlayer.play(src[0]); 
    }
    ...
    stopHandle () {
        this.resetCurrentTime();
        this.percentCallback(this.currentPercent())
        this.endCallback();
    }
