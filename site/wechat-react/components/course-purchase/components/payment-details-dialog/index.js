...
class PaymentDetailsDialog extends Component {
    ...
    // 自动选中可使用的最大优惠券
    async autoSelectCoupon (chargeConfig) {
        const {
            showDiscount,
            couponLists,
            businessType,
        ...
        const charge = chargeConfig || this.currCharge
        ...
        // 判断是否有平台通用券，有的话优先使用
        const isHavePlatformCoupon = couponLists.find(i => i.couponType === 'discount')
        const matchCouponList = couponLists.filter( coupon => 
...
export default connect(mapStateToProps, mapActionToProps)(PaymentDetailsDialog);