...
import { getPushInfo } from 'actions/channel';
import { autobind } from 'core-decorators';

@autobind
class PushChannelDialog extends PureComponent {
	...
	async getPushInfo() {
		if (this.props.channelId){
			let result = await this.props.getPushInfo({ channelId: this.props.channelId });
	...
	async push() {
		...
		setTimeout(() => {
			location.href = `/wechat/page/course/push/channel/${this.props.channelId}?sync=${this.state.timelineChecked ? 'Y' : 'N'}&pushQl=${this.state.pushQl ? 'Y' : 'N'}&pushSelf=${this.state.pushSelf ? 'Y' : 'N'}&liveId=${this.props.liveId}`
		}, 50);
	}
						...
						<ul className='select_ul'>
							{
								...
								<li  className='disable'>
									...
									千聊live服务号<i>（该系列课每周{this.state.qlchatInfo.weekSum}次机会，本周还剩{this.state.qlchatInfo.weekLeft}次）</i>
									{
										this.state.qlchatInfo.weekLeft <= 0?
										...
										<var className='tips'>直播间所有系列课1天只能推送{this.state.qlchatInfo.daySum}次！</var>
										:null	
									}
								</li>
								:
								<li onClick={() => { this.changeCheckbox('pushQl') }}>
									...
									千聊live服务号<i>（每周{this.state.qlchatInfo.weekSum}次机会，本周还剩{this.state.qlchatInfo.weekLeft}次）</i>
								</li>	
...
PushChannelDialog.defaultProps = {
	isShow: false
};

PushChannelDialog.propTypes = {
...
export default connect(mapStateToProps, mapActionToProps)(PushChannelDialog);
