...
import { fetchPushInfo } from '../../../check-in-camp/actions/mine';
import { autobind } from 'core-decorators';

@autobind
class PushCheckInCampDialog extends PureComponent {
	 ...
	 * 
	 * 获取打卡训练营推送信息
	 * @memberof PushCheckInCampDialog
	 */
	async fetchPushInfo() {
		if (this.props.campId){
			let result = await this.props.fetchPushInfo({ campId: this.props.campId });
	...
	async push() {
		...
		setTimeout(() => {
			location.href = `/wechat/page/course/push/camp/${this.props.campId}?sync=${this.state.timelineChecked ? 'Y' : 'N'}&pushQl=${this.state.pushQl ? 'Y' : 'N'}&pushSelf=${this.state.pushSelf ? 'Y' : 'N'}&liveId=${this.props.liveId}`
		}, 50);
	}
						...
						<ul className='select_ul'>
							{
								...
								<li  className='disable'>
									...
									千聊live服务号<i>（该训练营每周{this.state.qlchatInfo.totalNum}次机会，本周还剩{this.state.qlchatInfo.totalNum - this.state.qlchatInfo.pushedNum}次）</i>
									{
										...
										:(this.state.qlchatInfo.dayPushed >= this.state.qlchatInfo.daySum && this.state.qlchatInfo.dayPushed != 0 )?
										<var className='tips'>训练营1天只能推送{this.state.qlchatInfo.daySum}次！</var>
										:null	
									}
								</li>
								:
								<li onClick={() => { this.changeCheckbox('pushQl') }}>
									...
									千聊live服务号<i>（每周{this.state.qlchatInfo.totalNum}次机会，本周还剩{this.state.qlchatInfo.totalNum - this.state.qlchatInfo.pushedNum}次）</i>
								</li>	
...
PushCheckInCampDialog.defaultProps = {
	isShow: false
};

PushCheckInCampDialog.propTypes = {
...
export default connect(mapStateToProps, mapActionToProps)(PushCheckInCampDialog);
