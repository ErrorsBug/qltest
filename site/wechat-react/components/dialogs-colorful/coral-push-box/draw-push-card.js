...
var pageprintpic = 'https://img.qlchat.com/qlLive/channelLogo/8S169RKE-YEPJ-QLJJ-1514876383085-G316RTFZ2L3T.png';
var haibaopic= null;
var formateTime={
    date:'',
    day:'',
 ...
 * 生成sharecard图片
 */     
export function pushDistributionCardMaking (bgUrl, datas, cb, reset,stylemodule,cardWidth,cardHeight,thisofficialKey,shareKey) {
    shareData = datas;
...
function genContent (canvas, cb) {
    ...
    loadImage(function () {
        ...
        //headbg_style_page
        if(moduleData.headbg_style_page){
            ctx.save();

            if(moduleData.headbg_style_page.isClip){
                ctx.beginPath();
                moduleData.headbg_style_page.r&&headBgClipRounded(ctx,moduleData.headbg_style_page.x,moduleData.headbg_style_page.y,moduleData.headbg_style_page.w,moduleData.headbg_style_page.h,moduleData.headbg_style_page.r,moduleData.headbg_style_page.bdWidth,moduleData.headbg_style_page.bdColor,moduleData.headbg_style_page.bgcolor,moduleData.headbg_style_page.isLinear,moduleData.headbg_style_page.linearColor);
                ctx.clip();
            }

            ctx.drawImage(
                bgMap[encodeURIComponent(pageprintpic)],
                moduleData.headbg_style_page.left,
                moduleData.headbg_style_page.top,            
                moduleData.headbg_style_page.width,
                moduleData.headbg_style_page.height
            );
            ctx.restore();
            ctx.closePath();
        }
        ...
        /* 画二维码*/
        if(moduleData.qrObj.shadow){
            ctx.shadowOffsetX = moduleData.qrObj.shadow.x; // 阴影Y轴偏移
            ctx.shadowOffsetY = moduleData.qrObj.shadow.y; // 阴影X轴偏移
            ctx.shadowBlur = moduleData.qrObj.shadow.size; // 模糊尺寸
            ctx.shadowColor = moduleData.qrObj.shadow.color; // 颜色
        }
        ctx.drawImage(
            qrcode,
...
function loadImage (cb) {
    if (!qrcode) {
        ...
        if(shareData.url){
            let url=(/(officialKey|lshareKey)/.test(shareData.url)?shareData.url:`${shareData.url}?officialKey=${officialKey}${lshareKey?'&lshareKey='+lshareKey:''}`);
            qrcode.src = imageProxy("http://qr.topscan.com/api.php?text="+encodeURIComponent(url));
        }else{
            if(shareData.businessType==="CHANNEL"){
                qrcode.src = imageProxy("http://qr.topscan.com/api.php?text="+encodeURIComponent(window.location.origin+"/live/channel/channelPage/"+shareData.businessId+".htm?officialKey="+officialKey+"&lshareKey="+lshareKey));
            }else{
                qrcode.src = imageProxy("http://qr.topscan.com/api.php?text="+encodeURIComponent(window.location.origin+"/topic/details?topicId="+shareData.businessId+"&officialKey="+officialKey+"&lshareKey="+lshareKey));
            }

        }
        qrcode.onload = function () {
            console.log(bgMap);
            if (bgMap[encodeURIComponent(pageprintpic)]&&bgMap[encodeURIComponent(haibaopic)]) {
                cb()
            } else {
                ...
                headbg.onload = function () {
                    ...
                    var pageprint = new Image();
                    pageprint.crossOrigin = 'Anonymous';
                    pageprint.src = imageProxy(pageprintpic + '?x-oss-process=image/resize,h_469,w_750,m_fill,limit_0');
                    pageprint.onload = function () {
                        bgMap[encodeURIComponent(pageprintpic)] = pageprint;
                        cb();
                    }
                }
            }
