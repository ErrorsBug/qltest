            ...
            { props.isCompany && <i className="company" onClick={props.handleCompany} />}
            {
                props.symbolList.length>0&&props.symbolList.map((val, index) => {
                    ...
                    if(thekey==="hot" && !symbolMap.has("dai")){ //热心群众和代言人，优先显示代言人
                        return <Item {...val} thekey={thekey} handleBtnClick={props.handleHotClick} key={`${val.key}-${index}`} />
                    }else if(thekey==="dai"){
                        return <Item {...val} thekey={thekey} handleBtnClick={props.handleDaiClick} key={`${val.key}-${index}`} />
                    }else if(thekey==="livet" && !symbolMap.has("pyramid")){ // 有金字塔就不显示教师节
                        return <Item {...val} thekey={thekey} handleBtnClick={props.handleTClick} key={`${val.key}-${index}`} />
                    }else if(thekey==="livev"){
                        return <Item {...val} thekey={thekey} handleBtnClick={props.handleVClick} key={`${val.key}-${index}`} />
                    }else if(thekey==="doctor"){
                        return <Item {...val} thekey={thekey} handleBtnClick={props.handleDoctorBtnClick} key={`${val.key}-${index}`} />
                    }else if(thekey==="pyramid"){
                        return <Item {...val} thekey={thekey} handleBtnClick={props.showPyramidDialog} key={`${val.key}-${index}`} />
                    }
                })
            }
