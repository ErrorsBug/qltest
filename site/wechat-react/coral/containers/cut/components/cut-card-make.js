 ...
 * }
 */     
export function cutCardMake (bgUrl,datas, cb, reset,stylemodule,cardWidth,cardHeight) {
    styleModule= stylemodule;
    shareData = datas;//userInfo
    if (!canvas) {
...
function loadImage (cb) {
    ...
    if (!headImage) {
        headImage = new Image();
        headImage.src = imageProxy(shareData.headImgUrl + '?x-oss-process=image/resize,h_300,w_300,m_fill,limit_0');
        headImage.crossOrigin = 'Anonymous';
        headImage.onload = function () {
            if (!qrcode) {
                qrcode = new Image();
                qrcode.crossOrigin = 'Anonymous';
                qrcode.src = imageProxy("http://qr.topscan.com/api.php?text="+encodeURIComponent(shareData.domainUrl+"activity/page/cut-price?businessType="+shareData.businessType+"&businessId="+shareData.businessId+"&applyUserId="+shareData.applyUserId));
