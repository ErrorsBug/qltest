...
var headBgClipRounded = canvasFun.headBgClipRounded;
...
function drawCard ({
    ...
    generateImages(ctx, imageContens, () => {
        ...
        generateBlock(ctx, blockContents)
...
function generateBlock (ctx, blockContents) {
    if (!blockContents || blockContents.length === 0) return

    blockContents.forEach(blockObj => {
        headBgClipRounded(
            ctx,
            blockObj
        )
