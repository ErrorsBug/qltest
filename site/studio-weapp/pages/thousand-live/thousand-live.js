...
Page({
    ...
    onLoad(options) {
        ...
        app.login().then(() => {
            ...
            // 初始化直播间分销参数
            this.initLshareKey(options.lshareKey);
    ...
    fetchInitData(topicId, fromDetail) {
        return request({
            ...
            let topicName = encodeURIComponent(getVal(res, 'data.topicView.topicPo.topic') || '');
            ...
            if (res && res.data.code && (res.data.code == 10001 || res.data.code == 404)) {
                ...
                wx.redirectTo({
                    url: `/pages/intro-topic/intro-topic?topicId=${topicId}&lshareKey=${this.data.lshareKeyOfThire}`,
                });
                return;
                ...
                wx.redirectTo({
                    url: `/pages/web-page/web-page?url=${url}&title=${topicName}`
                });
                return;
                ...
                // return;
            } else if (fromDetail != 'Y' && diffTime > 2) {
                // 如果已开播两个小时就跳转到极简模式
                wx.redirectTo({
    ...
    updatePPTIndex() {
        ...
        for (var idx = 0, len = this.data.pptList.length; idx < len; idx++) {
            let ppt = this.data.pptList[idx];
